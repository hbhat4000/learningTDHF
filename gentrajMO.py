...
def HamCO(modelten,t,PCO,field):
    # compute two-electron term, result will be ntrain x drc x drc
    twoe = -cp.einsum('ijkl,kl->ij',modelten,PCO,optimize=True)
    ...
    if field:
        ...
        hfieldAO = ez*didat[2]
        H += -xmatCP.conj().T @ hfieldAO @ xmatCP
    return H

def MMUTHO_CP(theta, initial_density, dt=0.08268, ntvec=2000, field=False):
    eetenmodelCP = (BmatCombCP @ theta).reshape((drc,drc,drc,drc))
