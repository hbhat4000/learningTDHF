...
def _sym_ortho(a, b):
    ...
    """
    if b == 0:
        return cp.sign(a), 0, abs(a)
    elif a == 0:
        return 0, cp.sign(b), abs(b)
    elif abs(b) > abs(a):
        tau = a / b
        s = cp.sign(b) / sqrt(1 + tau * tau)
        c = s * tau
        r = b / s
    else:
        tau = b / a
        c = cp.sign(a) / sqrt(1+tau*tau)
        s = c * tau
        r = a / c
    return c, s, r

def culsmr(m, n, matvec, rmatvec, b, damp=0.0, atol=1e-6, btol=1e-6, conlim=1e8,
         maxiter=None, show=False, x0=None, outfname=None):
        ...
        if show:
            if (n <= 40) or (itn <= 10) or (itn >= maxiter - 10) or \
               ...
               (istop != 0):
                ...
                print(''.join([str1, str2, str3, str4]), flush=True)

        if itn % 100 == 0:
            numpy.savez(outfname,x=cp.asnumpy(x))
