...
if args.theta == 'true':
    print("Error! There is no truetheta for the PG model!")
...
beta1 = whichtheta.reshape((drc**2, drc**2))
# compute raw model predictions
predmat = (beta0CP + cp.matmul(ptrainCP.reshape((-1, drc**2)), beta1)).reshape((-1, drc, drc))
# separate out real and imag parts
predrmat = cp.real(predmat)
predqmat = cp.imag(predmat) 
# symmetrize
modh = 0.5*(predrmat + cp.transpose(predrmat,(0,2,1))) + 0.5j*(predqmat - cp.transpose(predqmat,(0,2,1)))
# [h, p]
predcomm = cp.matmul(modh,ptrainCP) - cp.matmul(ptrainCP,modh)
