...
for i in ${!mols[@]}; do
  ...
  python fitHB.py --gpu 0 --mol ${mols[$i]} --basis ${bases[$i]} --pagg ${paggfname} --pdotagg ${pdotaggfname} --dt 0.0008268 --tol 1e-16 --maxiter 100000 --restart ./HBthetas100/thetaR_${mols[$i]}_${bases[$i]}.npz --outfname ${outfname} > ${logfname}
done
